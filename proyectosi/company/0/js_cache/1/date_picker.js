...
this.setDate=setDate;function setDate(year,month,day){if(dateField){month=tmonths[month];var dateString=day+'/'+month+'/'+year;dateField.value=dateString;setFocus(dateField.name);if(dateField.getAttribute('aspect')=='cdate')
...
if(dateField){try{var dateString=new String(dateField.value);var dateParts=dateString.split('/');selectedDay=parseInt(dateParts[0],10);selectedMonth=parseInt(tmonths.indexOf(dateParts[1]),10);selectedYear=parseInt(dateParts[2],10);}catch(e){}
}
...
window.addEventListener('click',function(e){if(document.getElementById('CC').contains(e.target)){}else{if(cC.visible()){cC.hide();}
}
